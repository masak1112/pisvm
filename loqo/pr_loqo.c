...
int choldc(double a[], int n, double p[])
{
  ...
  for (i = 0; i < n; i++){
	...
	if (sum <= 0.0) {
	  ...
	  printf("choldc failed, matrix not positive definite"); fflush(stdout);
	  sum = 0.0;
	  return -1;
	}
	p[i]=sqrt(sum);
...
int solve_reduced(int n, int m, double h_x[], double h_y[], 
		  ...
		  double workspace[], int step)
{
  ...
  if (step == PREDICTOR) {
    ierr = choldc(h_x, n, p_x);	/* do cholesky decomposition */
    if(ierr)
      return ierr;
    ...
    ierr = choldc(h_y, m, p_y);	/* and cholesky decomposition */
    if(ierr)
      return ierr;
  }
...
/*   double primal_obj = *p_obj; */
/*   double dual_obj = *d_obj; */
  double mu;
  double alfa;
  int counter = 0;
  ...
  /* initial settings */
  c_plus_1 = 0; b_plus_1 = 0;
  for (i=0; i<n; i++) c_plus_1 += sqr(c[i]);
  for (i=0; i<m; i++) b_plus_1 += sqr(b[i]);
  c_plus_1 = sqrt(c_plus_1);
  c_plus_1 += 1;
  b_plus_1 = sqrt(b_plus_1);
  b_plus_1 += 1;
  ...
  if (restart == 1) {
    ...
    ierr = solve_reduced(n, m, h_x, h_y, a, x, y, c_x, c_y, workspace,
			 PREDICTOR);
    if(ierr)
      return ierr;
  ...
  while (status == STILL_RUNNING) {
    ...
    printf("sigfig = %g, sigfig_max = %g\n", sigfig, sigfig_max); fflush(stdout);
	...
	for (j=i; j<m; j++) 
      ...
      /* and do it */
      ierr = solve_reduced(n, m, h_x, h_y, a, delta_x, delta_y, c_x, c_y, 
			   workspace, PREDICTOR);
      if(ierr)
	return ierr;
