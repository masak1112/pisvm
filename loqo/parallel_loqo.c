...
void PLA_Set_unit_diagonal(PLA_Obj *a)
{
    ...
    while(1)
    {
        ...
        CheckError(info);
        info = PLA_Obj_local_length(a11, &local_m);
        CheckError(info);
        info = PLA_Obj_local_width(a11, &local_n);
        CheckError(info);
...
void print_vector(PLA_Obj v)
{
    ...
    info = PLA_Obj_local_length(v, &local_n);
    CheckError(info);
    info = PLA_Obj_local_stride(v, &local_stride);
    CheckError(info);
    info = PLA_Obj_local_buffer(v, (void **) &addr_buf);
    CheckError(info);
    for(i=0; i<local_n; ++i)
    {
...
void PLA_Copy_vector(PLA_Obj from, PLA_Obj to)
{
    ...
    int info;
    int local_stride, local_n, i;
    info = PLA_Obj_template(from, &templ);
    CheckError(info);
    info = PLA_Temp_comm_all(templ, &comm);
    CheckError(info);
    info = PLA_Obj_local_length(from, &local_n);
    CheckError(info);
    info = PLA_Obj_local_stride(from, &local_stride);
    CheckError(info);
    info = PLA_Obj_local_buffer(from, (void **) &from_buf);
    CheckError(info);
    info = PLA_Obj_local_buffer(to, (void **) &to_buf);
    CheckError(info);
    for(i=0; i<local_n; ++i)
    {
...
void PLA_Set_diagonal(PLA_Obj *a, PLA_Obj d)
{
    ...
    while(1)
    {
        ...
        info = PLA_Obj_local_width(a11, &local_n);
        CheckError(info);
        info = PLA_Obj_local_length(d1, &local_m2);
        CheckError(info);
                            ...
                            int step)
{
    ...
    info = PLA_Obj_template(Q1, &templ);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(*delta_alpha, 1, &Y2);
    CheckError(info);
    info = PLA_Mscalar_create(MPIfloat, PLA_ALL_ROWS, PLA_ALL_COLS, 1, 1,
                              templ, &plus_one);
    CheckError(info);
    info = PLA_Obj_set_to_one(plus_one);
    CheckError(info);
    info = PLA_Mscalar_create(MPIfloat, PLA_ALL_ROWS, PLA_ALL_COLS, 1, 1,
                              templ, &minus_one);
    CheckError(info);
    info = PLA_Obj_set_to_minus_one(minus_one);
    CheckError(info);
    if(step == PREDICTOR)
    {
        ...
        info = PLA_Trsm(PLA_SIDE_RIGHT, PLA_LOWER_TRIANGULAR, PLA_TRANS,
                        PLA_NONUNIT_DIAG, plus_one, Q1, *Y1T);
        CheckError(info);
    ...
    info = PLA_Trsv(PLA_LOWER_TRIANGULAR, PLA_TRANS, PLA_NONUNIT_DIAG, Q1,
                    *delta_alpha);
    CheckError(info);
    info = PLA_Obj_free(&Y2);
    CheckError(info);
    info = PLA_Obj_free(&plus_one);
    CheckError(info);
    info = PLA_Obj_free(&minus_one);
    CheckError(info);
}
                  ...
                  int counter_max, LOQOfloat margin, LOQOfloat bound, int restart)
{
    ...
    info = PLA_Obj_template(h_x, &templ);
    CheckError(info);
    info = PLA_Temp_comm_all_info(templ, &comm, &rank, &numnodes);
    CheckError(info);
    info = PLA_Obj_global_length(h_x, &n);
    CheckError(info);
    info = PLA_Obj_global_length(b, &m);
    CheckError(info);
    ...
    /* Allocation */
    info = PLA_Mvector_create_conf_to(c, 1, &diag_h_x);
    CheckError(info);
    ...
    info = PLA_Matrix_create(MPIfloat, n, n, templ, PLA_ALIGN_FIRST,
                             PLA_ALIGN_FIRST, &h_x_copy);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &c_x);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(b, 1, &c_y);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &h_dot_x);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(b, 1, &rho);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &nu);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &tau);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &sigma);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &gamma_z);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &gamma_s);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &hat_nu);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &hat_tau);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &delta_x);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(b, 1, &delta_y);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &delta_s);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &delta_z);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &delta_g);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &delta_t);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &d);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &ones_n);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(b, 1, &ones_m);
    CheckError(info);
    info = PLA_Mvector_create_conf_to(c, 1, &max_ratio);
    CheckError(info);
    ...
    info = PLA_Obj_local_length(*x, &local_n);
    CheckError(info);
    info = PLA_Obj_local_stride(*x, &local_stride);
    CheckError(info);
    info = PLA_Obj_local_length(b, &local_m);
    CheckError(info);
    info = PLA_Obj_local_stride(b, &local_stride2);
    CheckError(info);
    ...
    info = PLA_Obj_set_to_minus_one(minus_one);
    CheckError(info);
    info = PLA_Obj_set_to_one(ones_n);
    CheckError(info);
    info = PLA_Obj_set_to_one(ones_m);
    CheckError(info);
    info = PLA_Dot(c, c, c_plus_1);
    CheckError(info);
    info = PLA_Dot(b, b, b_plus_1);
    CheckError(info);
    ...
    *loc.b_plus_1 += 1.0;
    ...
    if(restart == 1)
    {
        ...
        info = PLA_Obj_set_to_zero(h_dot_x);
        CheckError(info);
        ...
        CheckError(info);
        info = PLA_Axpy(plus_one, c, sigma);
        CheckError(info);
    ...
    else
    {
        ...
        info = PLA_Obj_set_to_zero(h_y);
        CheckError(info);
        PLA_Set_unit_diagonal(&h_y);
        /* Set h_x <- h_x + n x n identity matrix */
        info = PLA_Obj_set_to_one(d);
        CheckError(info);
        info = PLA_Axpy(plus_one, diag_h_x, d);
        CheckError(info);
    ...
    *loc.mu = (*loc.mu + (*loc.tmp)) / (2*n);
    ...
    while(status == STILL_RUNNING)
    {
        ...
        info = PLA_Axpy(minus_one, *x, nu);
        CheckError(info);
        info = PLA_Axpy(plus_one, *g, nu);
        CheckError(info);
        /* Set tau <- u - x - t */
        PLA_Copy_vector(u, tau);
        info = PLA_Axpy(minus_one, *x, tau);
        CheckError(info);
        info = PLA_Axpy(minus_one, *t, tau);
        CheckError(info);
        /* Set sigma <- c - z + s + h_dot_x - A^T * y */
        PLA_Copy_vector(c, sigma);
        info = PLA_Axpy(minus_one, *z, sigma);
        CheckError(info);
        info = PLA_Axpy(plus_one, *s, sigma);
        CheckError(info);
        info = PLA_Axpy(plus_one, h_dot_x, sigma);
        CheckError(info);
        info = PLA_Gemv(PLA_TRANS, minus_one, a, *y, plus_one, sigma);
        /* Set gamma_z <- -z, gamma_s <- -s. */
        info = PLA_Obj_set_to_zero(gamma_s);
        CheckError(info);
        info = PLA_Axpy(minus_one, *s, gamma_s);
        CheckError(info);
        info = PLA_Obj_set_to_zero(gamma_z);
        CheckError(info);
        info = PLA_Axpy(minus_one, *z, gamma_z);
        CheckError(info);
        ...
        /*       print_vector(sigma); */
        /* Instrumentation */
        info = PLA_Dot(h_dot_x, *x, x_h_x);
        CheckError(info);
        {
            /* Compute
               primal_inf <- sqrt(tau^T * tau + nu^T * nu + rho^T * rho)
               / b_plus_1. */
            info = PLA_Dot(tau, tau, primal_inf);
            CheckError(info);
            info = PLA_Dot(nu, nu, tmp);
            CheckError(info);
            info = MPI_Barrier(comm);
            *loc.primal_inf += *loc.tmp;
            info = PLA_Dot(rho, rho, tmp);
            CheckError(info);
            info = MPI_Barrier(comm);
            *loc.primal_inf += *loc.tmp;
        ...
        }
        {
            ...
            info = PLA_Dot(sigma, sigma, dual_inf);
            CheckError(info);
            info = MPI_Barrier(comm);
        ...
        }
        {
            ...
            info = PLA_Dot(l, *z, dual_obj);
            CheckError(info);
            info = PLA_Dot(b, *y, tmp);
            CheckError(info);
            info = MPI_Barrier(comm);
            *loc.dual_obj += *loc.tmp;
            info = PLA_Dot(u, *s, tmp);
            CheckError(info);
            info = MPI_Barrier(comm);
            *loc.dual_obj -= *loc.tmp;
        ...
        if(status == 0) {
            ...
            info = PLA_Iamax(max_ratio, max_idx, alfa);
            CheckError(info);
            info = MPI_Barrier(comm);
            CheckError(info);
            ...
            *loc.mu = *loc.mu * sqr((*loc.alfa - 1) / (*loc.alfa + 10));
            ...
            /* Update variables. */
            info = PLA_Axpy(alfa, delta_x, *x);
            CheckError(info);
            info = PLA_Axpy(alfa, delta_g, *g);
            CheckError(info);
            info = PLA_Axpy(alfa, delta_t, *t);
            CheckError(info);
            info = PLA_Axpy(alfa, delta_z, *z);
            CheckError(info);
            info = PLA_Axpy(alfa, delta_s, *s);
            CheckError(info);
            info = PLA_Axpy(alfa, delta_y, *y);
            CheckError(info);
        } /* if(status == 0) */
    ...
    CheckError(info);
    info = PLA_Axpy(plus_one, c, *dist);
    CheckError(info);

    info = MPI_Barrier(comm);
    CheckError(info);
    /* Deallocation */
    info = PLA_Obj_free(&diag_h_x);
    CheckError(info);
    info = PLA_Obj_free(&h_y);
    CheckError(info);
    info = PLA_Obj_free(&y1t);
    CheckError(info);
    info = PLA_Obj_free(&c_x);
    CheckError(info);
    info = PLA_Obj_free(&c_y);
    CheckError(info);
    info = PLA_Obj_free(&h_dot_x);
    CheckError(info);
    info = PLA_Obj_free(&rho);
    CheckError(info);
    info = PLA_Obj_free(&nu);
    CheckError(info);
    info = PLA_Obj_free(&tau);
    CheckError(info);
    info = PLA_Obj_free(&sigma);
    CheckError(info);
    info = PLA_Obj_free(&gamma_z);
    CheckError(info);
    info = PLA_Obj_free(&gamma_s);
    CheckError(info);
    info = PLA_Obj_free(&hat_nu);
    CheckError(info);
    info = PLA_Obj_free(&hat_tau);
    CheckError(info);
    info = PLA_Obj_free(&delta_x);
    CheckError(info);
    info = PLA_Obj_free(&delta_y);
    CheckError(info);
    info = PLA_Obj_free(&delta_s);
    CheckError(info);
    info = PLA_Obj_free(&delta_z);
    CheckError(info);
    info = PLA_Obj_free(&delta_g);
    CheckError(info);
    info = PLA_Obj_free(&delta_t);
    CheckError(info);
    info = PLA_Obj_free(&d);
    CheckError(info);
    info = PLA_Obj_free(&ones_n);
    CheckError(info);
    info = PLA_Obj_free(&ones_m);
    CheckError(info);
    info = PLA_Obj_free(&h_x_copy);
    CheckError(info);
    info = PLA_Obj_free(&plus_one);
    CheckError(info);
    info = PLA_Obj_free(&minus_one);
    CheckError(info);
    info = PLA_Obj_free(&b_plus_1);
    CheckError(info);
    info = PLA_Obj_free(&c_plus_1);
    CheckError(info);
    info = PLA_Obj_free(&x_h_x);
    CheckError(info);
    info = PLA_Obj_free(&primal_inf);
    CheckError(info);
    info = PLA_Obj_free(&dual_inf);
    CheckError(info);
    info = PLA_Obj_free(&primal_obj);
    CheckError(info);
    info = PLA_Obj_free(&dual_obj);
    CheckError(info);
    info = PLA_Obj_free(&mu);
    CheckError(info);
    info = PLA_Obj_free(&alfa);
    CheckError(info);
    info = PLA_Obj_free(&tmp);
    CheckError(info);
